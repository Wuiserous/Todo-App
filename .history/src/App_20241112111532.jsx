...
function App() {
  ...
  const [showCardOptionModal, setShowCardOptionModal] = useState(false)
  ...
  const hideCardOptionModal = () => setShowCardOptionModal(false)
  return (
    <div className='w-[100vw] overflow-hidden p-2 gap-2 h-[100vh] grid grid-cols-[50px_1100px_60px_1fr] grid-rows-[50px_1fr_1fr_1fr]'>
      ...
      <TaskSpace Button={<button onClick={() => setShowCardOptionModal(true)} className="rounded border-none bg-white focus:outline-none bg-black p-2">
                    <BsThreeDotsVertical className="text-black" size={25} />
                </button>} deleteCard={deleteCard} cards={cards} bgColor={darkMode?'bg-slate-300' : 'bg-gray-800' } />
      ...
      <Modal Show={showModal} hide={hideModal} bgColor={darkMode?'bg-slate-300' : 'bg-gray-800'}>
        <AddTodo Button={<button onClick={() => setShowLinkModal(true)} className="rounded border-none bg-white focus:outline-none bg-black p-2">
                  ...
                  ></textarea>} submitButton={<button className="col-start-1 col-span-7 row-start-12 row-end-13 hover:bg-blue-700 bg-blue-500 p-2 rounded" onClick={handleSubmit}>Add</button>}
                  />
      </Modal>
      <cardOption Show={showLinkModal} hide={hideCardOptionModal}/>
