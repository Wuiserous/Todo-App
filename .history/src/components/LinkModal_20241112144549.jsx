...
export default function LinkModal(props) {
    ...
    return (
        ...
        onClick={hide}>
            <div className={`fixed top-60 left-[480px] w-[200px] p-2 h-fit backdrop-blur-lg transform transition-all duration-100 text-black flex gap-2 flex-wrap overflow-auto rounded ${isVisible ? "scale-100 opacity-100" : "scale-50 opacity-0"}`}>
                {props.children}
            </div>
        </div>
    )
