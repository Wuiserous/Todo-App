    ...
    <div className={`rounded-xl grid grid-cols-3 gap-2 p-2 hide-scrollbar overflow-auto col-start-2 row-start-2 row-end-5 ${props.bgColor}`}>
      <div className="h-full flex flex-col gap-2 col-span-1">
        {columns[0].map((card, index) => (
          <div className='w-full h-fit border-[1px] border-gray-500 flex flex-col gap-2 p-4 rounded-xl card-animation transition-all duration-300' key={index}><h3>{card.title}</h3><p>{card.description}</p><button onClick={() => setShowCardOptionModal(true)}><BsThreeDotsVertical /></button>{showCardOptionModal?? <div className=""></div>}<button onClick={() => props.deleteCard(index)}>delete</button></div>
        ))}
      </div>
      <div className="h-full flex flex-col gap-2 col-span-1">
