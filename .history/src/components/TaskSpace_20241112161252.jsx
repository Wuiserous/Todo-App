    ...
    <div className={`rounded-xl grid grid-cols-3 gap-2 p-2 hide-scrollbar overflow-auto col-start-2 row-start-2 row-end-5 ${props.bgColor}`}>
      <div className="h-full flex flex-col gap-2 col-span-1">
        {columns[0].map((card, index) => (
          <div className={`w-full relative h-fit border-[1px] ${card.isUrgent && card.isImportant ? 'border-red-600' : 
              ...
              card.isImportant ? 'border-blue-500' : ''} border-gray-500 ${index*3 === props.hoveredCardIndex ? 'bg-green-500' : ''} flex flex-col gap-2 p-4 rounded-xl card-animation transition-all duration-300`} key={index}>
            <h3>{card.title}</h3>
      ...
      <div className="h-full flex flex-col gap-2 col-span-1">
        {columns[1].map((card, index) => (
          <div className={`w-full relative h-fit border-[1px] ${card.isUrgent && card.isImportant ? 'bg-red-600' : 
            ...
            card.isImportant ? 'border-blue-500' : ''} border-gray-500 ${index*3 + 1 === props.hoveredCardIndex ? 'bg-green-500' : ''} flex flex-col gap-2 p-4 rounded-xl card-animation transition-all duration-300`} key={index}>
            <h3>{card.title}</h3>
            <p>{card.description}</p>
            <p>Priority: 
              {card.isUrgent && card.isImportant ? 'Urgent and Important' : 
      ...
      <div className="h-full flex flex-col gap-2 col-span-1">
        {columns[2].map((card, index) => (
          <div className={`w-full relative h-fit border-[1px] ${card.isUrgent && card.isImportant ? 'border-red-600' : 
            card.isUrgent ? 'border-orange-500' : 
            card.isImportant ? 'border-blue-500' : ''} border-gray-500 ${index*3 + 2 === props.hoveredCardIndex ? 'bg-green-500' : ''} flex flex-col gap-2 p-4 rounded-xl card-animation transition-all duration-300`} key={index}>
            <h3>{card.title}</h3>
            <p>{card.description}</p>
            <p>Priority: 
              {card.isUrgent && card.isImportant ? 'Urgent and Important' : 
