...
export default function DeadLineModal(props) {
    ...
    return (
        <div className="fixed w-full h-full inset-0 z-10" id="wrapper" onClick={hide}>
            <div className={`fixed flex flex-wrap gap-2 bg-[#1E1E1E] top-[81px] left-[50%] transform -translate-x-[50%] w-[200px] p-2 h-[200px] bg-white overflow-auto rounded transition-all duration-100 ${isVisible ? "scale-100 opacity-100" : "scale-50 opacity-0"}`}>
                <div className="">
                    {props.children}
                    <p>{props.time}</p>
                    <p>{props.date}</p>
