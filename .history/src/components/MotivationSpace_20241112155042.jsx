export default function MotivationSpace(props) {
  ...
  const portion1 = text.slice(0, 50)
  const portion2 = text.slice(50, 1000)
    return (
      <div className={`rounded-xl row-start-2 row-end-5 col-start-4 col-end-4 col-span-3 ${props.bgColor}`}>
        <div className="flex flex-col justify-center">
          <div className="flex"><span className="text-2xl">{name}: </span><div className="w-full"><p>{portion1}</p></div></div>
          <p>{portion2}</p>
        </div>
      </div>
    )
