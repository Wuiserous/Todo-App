export default function TaskSpace(props) {
  ...
  const cards = [{title: 'title 1', description: 'description 1'}, 'Card 2', 'Card 3', {title: 'this is a title of 4th card', description: 'i think the bento design is now looking awesome. lets just write the backend for it right now what we need here is the array that will contains the dicts of title and descriptions of individual cards set from the add todo component'}, 'Card 5', 'Card 6', 'Card 7', 'Card 8'];
    ...
    <div className={`rounded-xl grid grid-cols-3 gap-2 p-2 hide-scrollbar overflow-auto col-start-2 row-start-2 row-end-5 border ${props.bgColor} border-gray-500`}>
      <div className="h-full flex flex-col gap-2 col-span-1">
        {columns[0].map((card, index) => (
          <div className='w-full h-fit border p-2 rounded'key={index}><h4>{card.title}</h1><p>{card.description}</p></div>
        ))}
      </div>
      <div className="h-full flex flex-col gap-2 col-span-1">
