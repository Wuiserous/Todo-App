...
export default function AddTodo(props) {
    ...
    console.log(`dateString says: ${dateString}`)
    console.log(`timeString says: ${timeString}`)
            ...
            {showTask && (
                <div className="grid auto-rows-auto gap-2 grid-cols-[1fr_1fr_1fr_1fr_1fr_1fr_1fr_1fr_1fr_1fr]">
                    <div className="row-span-1 col-span-10 border flex flex-row justify-center items-center rounded-lg rounded-br-[0px] bg-[#ffffff] pr-1">
                        {props.titleInput}
                        {if (day != '')}
                        <div className="w-fit h-fit bg-orange-500 rounded-lg flex flex-row gap-1 p-1 rounded-br-[0px]"><span>{day}</span><span>{month}</span><span>{time}</span></div>
                    </div>
