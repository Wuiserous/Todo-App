        ...
        {columns[0].map((card, index) => (
          <div className={`w-full relative h-fit border-[1px] ${card.isUrgent && card.isImportant ? 'border-red-600' : 
            ...
            {card.isUrgent && card.isImportant ? (
              ...
              card.isUrgent ? '🔥' : 
              card.isImportant ? '⭐' : (
                <div className="absolute top-0 right-0 w-1 h-0 border-t-[50px] border-r-[50px] border-t-transparent border-r-transparent border-l-blue-500 border-b-blue-500"></div>
              )}
          </div>
        ))}
      </div>
