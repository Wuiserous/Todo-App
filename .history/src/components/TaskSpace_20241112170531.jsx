      ...
      <div className="h-full flex flex-col gap-2 col-span-1">
        {columns[0].map((card, index) => (
          <div className={`w-full relative h-fit border-[1px] ${card.isUrgent && card.isImportant ? 'border-red-600' : 
            ...
            <div className="absolute bottom-0 right-0 w-full h-full bg-white rounded-xl flex flex-col gap-2 p-4 transition-all duration-300"><button className="w-full h-fit bg-blue-500 p-2 rounded-xl" onClick={() => props.deleteCard(index*3 + 0)}>delete</button></div>
          </div>
        ))}
      </div>
      <div className="h-full flex flex-col gap-2 col-span-1">
