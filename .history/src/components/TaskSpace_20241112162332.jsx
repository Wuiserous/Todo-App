      ...
      <div className="h-full flex flex-col gap-2 col-span-1">
        {columns[0].map((card, index) => (
          <div className={`w-full relative h-fit border-[1px] ${card.isUrgent && card.isImportant ? 'bg-red-600' : 
              ...
              card.isImportant ? 'bg-blue-500' : ''} border-gray-500 ${index*3 === props.hoveredCardIndex ? 'bg-green-500' : ''} flex flex-col gap-2 p-4 rounded-xl card-animation transition-all duration-300`} key={index}>
            <h3>{card.title}</h3>
