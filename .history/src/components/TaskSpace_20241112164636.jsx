        ...
        {columns[0].map((card, index) => (
          <div className={`w-full relative h-fit border-[1px] ${card.isUrgent && card.isImportant ? 'bg-red-600' : 
            ...
            {index*3 === props.hoveredCardIndex ? (
              <div className="w-full h-full absolute inset-0 bg-white text-black flex flex-col gap-2 p-4"><h3>{card.title}</h3>
        ...
        {columns[1].map((card, index) => (
          <div className={`w-full relative h-fit border-[1px] ${card.isUrgent && card.isImportant ? 'bg-red-600' : 
            ...
            {index*3 + 1 === props.hoveredCardIndex ? (
              <div className="w-full h-full absolute inset-0 bg-white rounded-xl text-black flex flex-col gap-2 p-4"><h3>{card.title}</h3>
            <p>{card.description}</p></div>
            ) : (null)}
            <h3>{card.title}</h3>
            <p>{card.description}</p>
        ...
        {columns[2].map((card, index) => (
          <div className={`w-full relative h-fit border-[1px] ${card.isUrgent && card.isImportant ? 'bg-red-600' : 
            ...
            {index*3 + 2 === props.hoveredCardIndex ? (
              <div className="w-full h-full absolute inset-0 bg-white rounded-xl flex flex-col gap-2 p-4"><h3>{card.title}</h3>
            <p>{card.description}</p></div>
            ) : (null)}
            <h3>{card.title}</h3>
            <p>{card.description}</p>
