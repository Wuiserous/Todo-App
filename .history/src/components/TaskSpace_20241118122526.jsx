        ...
        {columns[0].map((card, index) => (
          <div className={`w-full relative h-fit border-[1px] border-[#333333] bg-[#1E1E1E] hover:shadow-[0_0_15px_5px_rgba(187,134,252,0.5)] hover:border-white  flex flex-col gap-2 p-4 rounded-[5px] card-animation transition-all duration-300 group`} key={index}>  
            ...
            {card.isUrgent && card.isImportant ? (
              ...
              <div className="w-10 h-[5px] absolute z-10 bg-gray-300 bottom-[0px] rounded-bl-lg rounded-br-[0px] left-[0.1px] text-center text-[12px]">
                <span className="hidden group-hover:block">{card.priority ? card.priority : 'P4'}</span>
              </div>
          ) : null }
            <div className="absolute bottom-[-50px] right-[-1px]"><ProgressBar createdTime={card.createdAt} deadlineDate={card.deadLineDate} deadlineTime={card.deadLineTime} /></div>
          </div>
        ))}
      </div>
      <div className="h-full flex flex-col gap-2 col-span-1">
