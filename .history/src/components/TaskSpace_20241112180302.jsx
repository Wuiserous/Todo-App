        ...
        {columns[0].map((card, index) => (
          <div className={`w-full relative z-1 h-fit border-[1px] ${card.isUrgent && card.isImportant ? 'border-red-600' : 
            ...
            {card.isUrgent && card.isImportant ? (
              <div className="w-10 h-10 absolute -z-10 bg-red-600 top-0 right-[-10px]"></div>
            ) : 
              card.isUrgent ? '🔥' : 
              card.isImportant ? '⭐' : (
                <div className="w-10 h-10 absolute -z-1 0bg-red-600 top-[-10px] rounded-full right-[-10px]"></div>
              )}
          </div>
        ))}
      </div>
