...
export default function TaskSpace(props) {
  ...
  return (
    <div className={`rounded-xl relative grid grid-cols-3 gap-2 p-2 hide-scrollbar overflow-auto col-start-2 row-start-2 row-end-5 ${props.bgColor}`}>
      <button className="absolute top-2 right-2 bg-white p-1 rounded-full hover:bg-blue-500 z-100">open</button>
      <div className="h-full absolute flex flex-col gap-2 col-span-1">
        {columns[0].map((card, index) => (
          <div className={`w-full absolute h-fit border-[1px] border-[#333333] bg-[#1E1E1E] hover:shadow-[0_0_15px_5px_rgba(187,134,252,0.5)]  flex flex-col gap-2 p-4 rounded-[5px] rounded-br-[0px] rounded-bl-[0px] card-animation transition-all duration-300 group`} key={index}>  
