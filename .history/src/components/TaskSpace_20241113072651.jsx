        ...
        {columns[0].map((card, index) => (
          <div className={`w-full relative h-fit border-[1px] ${card.isUrgent && card.isImportant ? 'border-red-600' : 
            ...
            {card.isUrgent && card.isImportant ? (
              ...
              card.isUrgent ? '🔥' : 
              card.isImportant ? '⭐' : (
                <div className="w-10 h-5 absolute bg-red-600 bottom-[-20px] rounded-bl-xl rounded-br-xl right-[-1px]"></div>
              )}
          </div>
        ))}
      </div>
        ...
        {columns[1].map((card, index) => (
          <div className={`w-full relative h-fit border-[1px] ${card.isUrgent && card.isImportant ? 'border-red-600' : 
            ...
            {index*3 + 1 === props.hoveredCardIndex ? (
              <div className="w-full h-full absolute inset-0 bg-white rounded-xl border-t-[1px] border-gray-500 text-black flex flex-col gap-2 p-4 transition-all duration-300"><h3>{card.title}</h3>
            <p>{card.description}</p></div>
            ) : (null)}
            <h3>{card.title}</h3>
            <p>{card.description}</p>
