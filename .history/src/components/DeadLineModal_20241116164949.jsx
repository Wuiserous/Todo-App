...
export default function DeadLineModal(props) {
    ...
    return (
        ...
        onClick={hide}>
            <div className={`fixed top-[81px] left-[585px] w-[200px] p-2 h-fit bg-[#1E1E1E] transform transition-all duration-200 ease-in-out text-black overflow-auto rounded ${isVisible ? "scale-100 opacity-100 translate-x-0 translate-y-0" : "scale-50 opacity-0 translate-x-[-100%] translate-y-[100%]"} ${isClosing ? "scale-50 opacity-0 translate-x-[-100%] translate-y-[100%]" : "s"} flex flex-wrap gap-2 justify-center items-center`}>
                {props.children}
                {day && time ? (
