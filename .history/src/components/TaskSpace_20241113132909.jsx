    ...
    <div className={`rounded-xl grid grid-cols-3 gap-2 p-2 hide-scrollbar overflow-auto col-start-2 row-start-2 row-end-5 ${props.bgColor}`}>
      <div className="h-full flex flex-col gap-2 col-span-1">
        {columns[0].map((card, index) => (
          <div className={`w-full relative z-11 h-fit border-[1px] border-[#333333] bg-[#1E1E1E]  flex flex-col gap-2 p-4 rounded-tl-xl rounded-tr-xl rounded-bl-xl card-animation transition-all duration-300`} key={index}>  
            {index*3 === props.hoveredCardIndex ? (<div className='w-full h-full rounded-xl absolute z-0 inset-0 bg-green-500 shadow-xl shadow-[0_0_10px_5px_rgba(34,197,94,0.5)]'></div>) : (null)} 
