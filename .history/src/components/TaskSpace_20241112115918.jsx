      ...
      <div className="h-full flex flex-col gap-2 col-span-1">
        {columns[0].map((card, index) => (
          <div className='w-full relative h-fit border-[1px] border-gray-500 flex flex-col gap-2 p-4 rounded-xl card-animation transition-all duration-300' key={index}>
            ...
            <button onClick={() => toggleCardOptions(index)} className="absolute bottom-0 right-0 p-2 focus:outline-none">
                <BsThreeDotsVertical />
            </button>
            {showCardOptionModal[index] && (
                <div className="w-10 h-10 absolute bottom-[-50px] right-[-20px] bg-white flex items-center justify-center" id={index}>
                  <button className="text-black" onClick={() => props.deleteCard(index)}>delete</button>
                </div>
              )}
            </div>
        ))}
