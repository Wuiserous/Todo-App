export default function MotivationSpace(props) {
  ...
  const portion1 = text.slice(0, 32)
  const portion2 = text.slice(30, 1000)
    return (
      <div className={`rounded-xl row-start-2 row-end-5 col-start-4 col-end-4 col-span-3 ${props.bgColor}`}>
        <div className="flex flex-col border justify-center p-1">
          <div className="flex"><div className="text-2xl">{name}:</div><div className="w-full flex justify-center items-end"><p>{portion1}</p></div></div>
          <p>{portion2}</p>
        </div>
      </div>
    )
