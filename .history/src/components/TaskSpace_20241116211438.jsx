    ...
    <div className={`rounded-xl grid grid-cols-3 gap-2 p-2 hide-scrollbar overflow-auto col-start-2 row-start-2 row-end-5 ${props.bgColor}`}>
      <div className="h-full flex flex-col gap-2 col-span-1">
        {columns[0].map((card, index) => (
          <div className={`w-full relative h-fit border-[1px] border-[#333333] bg-[#1E1E1E] hover:shadow-[0_0_15px_5px_rgba(187,134,252,0.5)] hover:border-white  flex flex-col gap-2 p-4 rounded-tl-xl rounded-tr-xl rounded-bl-xl card-animation transition-all duration-300`} key={index}>  
            ...
            {card.isUrgent && card.isImportant ? (<div className="w-10 h-5 absolute z-10 bg-[#D32F2F] bottom-[-21px] rounded-bl-lg rounded-br-xl right-[-1px] text-center text-[12px]">{card.priority}</div>) : 
              card.isUrgent ? (<div className="w-10 h-5 absolute z-10 bg-blue-500 bottom-[-21px] rounded-bl-lg rounded-br-xl right-[-1px] text-center text-[12px]">{card.priority}</div>) : 
              card.isImportant ? (<div className="w-10 h-5 absolute z-10 bg-yellow-400 bottom-[-21px] rounded-bl-lg rounded-br-xl right-[-1px] text-center text-[12px]">{card.priority}</div>) : (
                <div className="w-10 h-5 absolute z-10 bg-[#03A9F4] bottom-[-21px] rounded-bl-lg rounded-br-xl right-[-1px] text-center text-[12px]">{card.priority}</div>
              ): card.isUrgent === false && card.isImportant === false ? (<div className="w-10 h-5 absolute z-10 bg-gray-300 bottom-[-21px] rounded-bl-lg rounded-br-xl right-[-1px] text-center text-[12px]">{card.priority}</div>) : (
          </div>
        ))}
      </div>
      <div className="h-full flex flex-col gap-2 col-span-1">
        {columns[1].map((card, index) => (
          <div className={`w-full relative h-fit border-[1px] border-[#333333] bg-[#1E1E1E] hover:shadow-[0_0_15px_5px_rgba(187,134,252,0.5)] hover:border-white flex flex-col gap-2 p-4 rounded-tl-xl rounded-tr-xl rounded-bl-xl card-animation transition-all duration-300`} key={index}>
            ...
            {card.isUrgent && card.isImportant ? (<div className="w-10 h-5 absolute z-10 bg-[#D32F2F] bottom-[-21px] rounded-bl-lg rounded-br-xl right-[-1px] text-center text-[12px]">{card.priority}</div>) : 
              card.isUrgent ? (<div className="w-10 h-5 absolute z-10 bg-[#FF5722] bottom-[-21px] rounded-bl-lg rounded-br-xl right-[-1px] text-center text-[12px]">{card.priority}</div>) : 
              card.isImportant ? (<div className="w-10 h-5 absolute z-10 bg-[#FF9800] bottom-[-21px] rounded-bl-lg rounded-br-xl right-[-1px] text-center text-[12px]">{card.priority}</div>) : (
                <div className="w-10 h-5 absolute z-10 bg-[#03A9F4] bottom-[-21px] rounded-bl-lg rounded-br-xl right-[-1px] text-center text-[12px]">{card.priority}</div>
              )}
          </div>
        ))}
