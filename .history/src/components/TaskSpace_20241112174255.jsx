        ...
        {columns[0].map((card, index) => (
          <div className={`w-full relative h-fit border-[1px] ${card.isUrgent && card.isImportant ? 'border-red-600' : 
            ...
            {index*3 === props.hoveredCardIndex ? (
              <div className="w-full h-full absolute inset-0 bg-white rounded-xl text-black flex flex-col gap-2 p-4 transition-all duration-300"><div class={`triangle_dash`}></div></div>
            ) : (null)}    
            <h3>{card.title}</h3>
