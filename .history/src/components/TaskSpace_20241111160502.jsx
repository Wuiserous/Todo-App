export default function TaskSpace(props) {
  ...
  const cards = ['skdf  skfsefd dkfjdfkj sldkfj slfkjoosoeaw oeafijeooeirfjjeoagir;gdlfejarogi aeoirjgaeio eoirajg er jgerkgjoeagjerigj oeajgierklgjdfgreogijagk dgjoer dlkg dfgjero fdkg dlgjero', 'Card 2', 'Card 3', 'Card 4', 'Card 5', 'Card 6', 'Card 7', 'Card 8'];
    ...
    <div className={`rounded-xl grid grid-cols-3 gap-4 p-2 hide-scrollbar overflow-auto col-start-2 row-start-2 row-end-5 border ${props.bgColor} border-gray-500`}>
      <div className="h-full col-span-1 border">
        {columns[0].map((card, index) => (
          <div className='w-full h-fit border'key={index}>{card}</div>
        ))}
      </div>
      <div className="h-full col-span-1 border">
