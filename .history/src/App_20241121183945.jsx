...
function App() {
  ...
  function handleSubmit(e) {
    ...
    const deadLineDate = {deadLineDate}; // Example input
    const deadLineTime = {deadLineTime}; // Example input

    // Parse the date and time
    const [day, month, year] = deadLineDate.split("/"); // Split the date
    const [time, period] = deadLineTime.split(" "); // Split the time and AM/PM
    const [hours, minutes, seconds] = time.split(":").map(Number); // Split and convert time to numbers
    ...
    const isoDateTime = `${year}-${month.padStart(2, "0")}-${day.padStart(2, "0")}T${adjustedHours
      .toString()
    ...
    const deadlineDateTime = new Date(isoDateTime);
    console.log("Deadline DateTime:", deadlineDateTime);
